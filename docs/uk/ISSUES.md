...
| GT-024 | Заблокована зовнішньою квотою; regression candidate не підтверджена | 1 | Після promotion v56 Web App показав network error; після exact rollback такий самий error відтворився на v55 | Apps Script Executions підтвердили завершені `doPost`/session/`mailboxRpc` для обох версій і одночасний `URLFETCH` daily-quota failure у Gmail API worker. Зберегти stable v55 та immutable staging v56; повторити контрольований A/B лише після відновлення квоти |
