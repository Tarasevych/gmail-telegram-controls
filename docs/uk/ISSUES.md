...
| GT-024 | PARTIAL; shared quota blocker повторився 2026-07-23 | 1 | Той самий generic mailbox error може вражати owner-only candidate і прийнятий production baseline під час вичерпання добової `URLFETCH` квоти | Історичне відновлення v57 лишається verified. Під час gate v70 staging один раз відкрився, але secondary switch впав; fresh production v65 launch потім повернув той самий error. Apps Script завершив `doPost`, redeem і RPC, тоді як timer впав у `legacy_recovery` з `errorCode=urlfetch_quota`. Candidate-specific regression не доведена; один bounded A/B повторювати лише після відновлення квоти |
...
- **Native результат v70:** staging mailbox відкрився без OAuth або повторного connection screen, відновив cached thread і показав avatar та три isolated Gmail roots. Перемикання на secondary root повернуло generic mail-operation error.
- **Контрольований A/B:** fresh production v65 launch повернув той самий generic error до завантаження mailbox. Apps Script показав завершені `doPost`, `mailboxRedeemLaunch` і `mailboxRpc`; сусідній timer execution впав у `legacy_recovery` з `errorCode=urlfetch_quota` та daily `urlfetch` quota exception. Це підтверджує shared blocker і не доводить v70-specific regression.
...
## 2026-07-23: продовження GT-051 для збереження чернеток
