...
## B1-30 — Стабільна позиція та фокус читача під час фонових оновлень

- **Статус:** PARTIAL.
- **Source request:** `REQ-0035`; V3 task `A-03`.
- **Локально VERIFIED:** no-op state не замінює root; необхідний render зберігає stable content anchor, viewport offset, bottom pin і memory-only focus identity; відкладені layout changes узгоджуються без reading-progress loop.
- **Gates:** focused reader contracts `8/8`, пов'язані contracts `101/101`, повний Apps Script suite `540/540`, clean diff check і `0` secret-signature matches.
- **Ще потрібно:** native desktop і mobile acceptance, перевірка long real HTML та remote-image layout, readback scroll під час return navigation і окремо авторизований cumulative staging/production acceptance.
- **Release boundary:** source commit `1d7c6c1`; immutable, staging, production, OAuth, Gmail або Telegram state у цьому контурі не змінено.
- **Доказ:** [GT-050](ISSUES.md), [VR-020](verification-reports/reports/VR-020/README.md).
- **English mirror:** [docs/en/ROADMAP.md](../en/ROADMAP.md).
