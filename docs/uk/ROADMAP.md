...
- **Статус:** VERIFIED
- **Випуск:** immutable v63 є production і HEAD; staging `0`.
- **Завершено:** deterministic reproduction, tokenized seven-minute crash lease, 150-second soft stage deadline, token-matched release, focused `17/17`, cumulative `501/501`, owner-only staging, два production launches і сім послідовних no-overlap runtime executions.
- **Доказ:** [VR-011](verification-reports/reports/VR-011/README.md) і [звіт про v63](reports/VERSIE_001_V63_RELEASE_AND_GT030_CLOSURE_2026-07-22.md).
- **Залишкова межа:** History-substage runtime telemetry та external automatic INBOX після v63 є окремими evidence items, а не blockers доведеного worker no-overlap gate.
...
- **Статус:** PARTIAL
- **Production boundary:** cumulative P0 source тепер deployed як immutable v63; native staging/production mailbox і account-isolation acceptance пройдено.
- **Перевірено:** dynamic active context, три isolated roots, account switching без OAuth, production app-shell load, worker no-overlap gate та exact release-state cleanup.
- **Поточний source step:** GT-031 тепер має tested narrow-screen full-address disclosure (`88/88` focused, `501/501` full); staging і production visual acceptance ще потрібні.
- **Поточний release step:** cumulative v64 helper pins source `da8b2768323db8fd8c1ba886b556bbfd2148d6de` і додає bounded GT-037 reconciliation; local release contracts зелені, але v64 immutable ще не існує.
- **Ще потрібно:** measured cold/warm і `A -> B -> A` traces; scroll/focus restoration; incremental arrival evidence; quota/LRU eviction; offline/restart/cross-session draft recovery; conflict handling; same-scale production typography comparison; stale-open-client exactly-one-reload/no-loop acceptance.
- **Пов’язані проблеми:** GT-031–GT-038.
- **Правило:** продовжувати в межах Versie 1. Не створювати наступний immutable candidate, доки code change цього не вимагає і чинна release operation не має terminal state.
- **Доказ:** [VR-009](verification-reports/reports/VR-009/README.md), [VR-010](verification-reports/reports/VR-010/README.md), [VR-011](verification-reports/reports/VR-011/README.md), [VR-012](verification-reports/reports/VR-012/README.md) і [VR-013](verification-reports/reports/VR-013/README.md).
