...
- **Статус:** production лишається exact v55; immutable v56 збережено як історію; immutable v57 має один owner-only staging deployment; A/B gate заблокований спільною зовнішньою `URLFETCH` quota.
...
- **Gates:** повний suite пройшов `443/443`, hash-pinned v57 `PreflightOnly` пройшов, owner-only staging v57 існує; production acceptance та account-switch A/B очікують відновлення зовнішньої квоти.
...
- **Статус:** заблоковано зовнішньою квотою; подальший code fix не створюється без доказу.
- **Safe state:** stable і HEAD v55; immutable v56 є історичним; один owner-only staging v57 збережено; Telegram menu вказує на production.
- **Діагноз:** v57 callback/redemption запуску/`mailboxRpc` завершилися, тоді як worker записав OAuth refresh failure і exact daily `urlfetch` exhaustion; той самий UI error відтворився у двох production v55 launches, тому regression v57 не підтверджена.
- **Наступний gate:** після відновлення daily quota два свіжі production v55 mailbox launches без network error, потім signed staging v57 з avatar, трьома roots, switch на контрольований другий account і назад без OAuth.
- **Release rule:** якщо помилка спільна, релізи повторно не перемикати; якщо доведено новий candidate-only defect, створити cumulative immutable v58 без переписування v56/v57 і зі збереженим exact v55 rollback.
- **Source requests:** REQ-0019, REQ-0021.

### B1-20 — Owner-only Advanced Gmail read adapter

- Додати allowlisted adapter для `messages.list`, `messages.get` і `history.list` під protected property `GMAIL_OWNER_ADVANCED_READ_V1=enabled`.
...
- **Межа доказу:** детерміновані adapter-тести проходять 8/8. Повний suite має 451/452, бо immutable v57 hash gate правильно відхиляє змінений `Code.gs`; цю branch не можна merge або deploy без окремого дозволу на наступний immutable. Flag лишається невстановленим, production лишається v55, а E4/E5 зменшення quota не перевірено.
