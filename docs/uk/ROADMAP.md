...
- **Статус:** immutable v56 збережено; production повернуто exact v55; A/B gate заблокований спільною зовнішньою `URLFETCH` quota.
...
- **Safe state:** stable і HEAD v55; immutable v56 та один owner-only staging v56 збережено; journal `rolled_back`; menu повернуто на production.
...
- **Наступний gate:** два свіжі production v55 mailbox launches без network error, потім signed staging v56 з avatar, трьома roots, switch на контрольований другий account і назад без OAuth.
- **Release rule:** якщо помилка спільна, релізи повторно не перемикати; якщо доведено code-level defect, створити cumulative immutable v57 без переписування v56 і зі збереженим exact v55 rollback.
