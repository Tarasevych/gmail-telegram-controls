...
- **Локально VERIFIED:** boot-first recovery app refresh credential через Telegram `SecureStorage`; memory-only bearer; single-flight client recovery; bounded idempotent server rotation для конкурентних tabs; content-free failure classification; terminal-only secure cleanup; explicit revoke boundary.
- **Gates:** focused auth/session contracts пройшли, повний Apps Script suite `561/561`, clean diff check і `0` credential-signature matches.
- **Ще потрібно:** native Telegram Desktop/mobile hard reload і WebView reopen, перевірка двох одночасних launches, відсутність повторного connection screen, staging acceptance та окремо дозволений production promotion.
- **Platform boundary:** автоматичний `restoreItem` не використовується, бо Telegram може запросити user confirmation; браузери без Telegram `SecureStorage` переходять до чинного fail-closed launch flow.
- **Release boundary:** source commit `975785a`; immutable, staging, production, OAuth, Gmail або Telegram state не змінено.
