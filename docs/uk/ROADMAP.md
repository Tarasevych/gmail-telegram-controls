...
- **Статус:** PARTIAL
- **Production boundary:** cumulative source deployed як immutable v64; staging `0`, journal `cleaned`, exact v63 є rollback.
...
- **Поточний source step:** GT-036 тепер читає canonical release-state field і має exact source marker v65; real-manifest one-reload/no-loop regression contract локально verified. Immutable або staging v65 ще не існує.
- **Ще потрібно:** source PR і bounded staging/production acceptance v65 для GT-036; measured cold/warm і `A -> B -> A` traces; scroll/focus restoration; incremental arrival evidence; quota/LRU eviction; offline/restart/cross-session draft recovery; conflict handling; same-scale production typography comparison.
- **Пов'язані проблеми:** GT-032–GT-036 і GT-038 лишаються open/partial; GT-031 і GT-037 verified.
...
- **Доказ:** [VR-009](verification-reports/reports/VR-009/README.md), [VR-013](verification-reports/reports/VR-013/README.md) і [VR-014](verification-reports/reports/VR-014/README.md).
