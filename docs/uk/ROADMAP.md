...
## B1-33 — Persistent app session після reload без повторного OAuth

- **Статус:** PARTIAL.
- **Source request:** `REQ-0036`; P0 session continuity.
- **Локально VERIFIED:** boot-first recovery app refresh credential через Telegram `SecureStorage`; memory-only bearer; single-flight client recovery; bounded idempotent server rotation для конкурентних tabs; content-free failure classification; terminal-only secure cleanup; explicit revoke boundary; повний Apps Script suite `561/561`.
- **Native result:** production v65 двічі завантажила mailbox; staging v69 завантажила mailbox після bounded repeat, але hard reload у Telegram Desktop повторно подав POST document і завершився `UNTRUSTED_NONCE_REPLAY`. Приблизно `20 s` до придатного staging UI також не відповідає one-second SLO; це ручне спостереження, не p95 benchmark.
- **Ще потрібно:** спроєктувати безпечний fallback для Telegram Desktop без придатного `SecureStorage` або підтвердити іншу platform-supported recovery модель; зберегти fail-closed replay protection; додати storage error telemetry; перевірити mobile/WebView reopen, два одночасні native launches і вимірюваний one-second acceptance.
- **Platform boundary:** автоматичний `restoreItem` не використовується, бо Telegram може запросити user confirmation. Перевірений Windows Desktop не надав usable secure recovery, а app wrapper не зберіг точний platform error code. Незахищені web-storage credentials не є допустимим fallback.
- **Release boundary:** source commit `975785a` увійшов до immutable v69. Candidate abandoned, exact staging видалено, journal terminal `abandoned`, owner menu production, active staging `0`; production verified v65 не змінено.
- **Доказ:** [GT-053](ISSUES.md), [VR-023](verification-reports/reports/VR-023/README.md).
- **English mirror:** [docs/en/ROADMAP.md](../en/ROADMAP.md).
