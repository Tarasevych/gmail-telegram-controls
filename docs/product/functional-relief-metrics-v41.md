...
- A later connected-Chrome run passed the complete desktop interaction. Its mobile viewport control then mis-scaled coordinates, so the already-authorized standalone Playwright fallback repeated the exact flow at 1440×900 and 390×844. All 28 checks pass: page identity, non-blank rendering, no framework overlay, bounded panel/card, no horizontal overflow, first-click panel persistence, inline confirm/cancel, cancel restoration, confirmed history clear, opt-in preservation, mobile panel scrolling, and console health.
- Private rendered evidence remains outside Git: `v41-functional-metrics-postfix-qa.json`, `v41-functional-metrics-postfix-desktop-confirm.png`, and `v41-functional-metrics-postfix-mobile-confirm.png` in the thread visualization directory. Only expected Telegram SDK v6 localhost warnings were observed.

Production remains Apps Script v33/product v38. This v41 slice is local and must not be deployed without a separately pinned immutable release gate and fresh Telegram WebView acceptance.
