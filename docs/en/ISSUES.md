...
| GT-024 | Open shared blocker | 1 | Web App bootstrap returned the same network error on production v56 and after exact rollback to v55 | A candidate-specific regression is not established; do not switch releases again until a controlled A/B test after the external quota resets |
