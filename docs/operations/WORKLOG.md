...
- Verification: targeted MailApp/MailClient 217/217 PASS; ordinary mutable-product matrix 365/365 PASS. A later connected-Chrome desktop pass and exact standalone Playwright fallback close the post-fix rendered gate with 28/28 checks at 1440×900 and 390×844. Clear-confirm/cancel, confirmed zero state, opt-in preservation, panel scrolling, bounds, overflow, and console health all pass; no release was attempted.
...
## 2026-07-18 — production Mini App session-capacity recovery

- User reproduced a 24/24 active-session failure in production v32/product v38. Live aggregate inspection found 23 owner families and one other-user family; the old close-window advice was confirmed false because WebView close has no server revocation hook.
- Ran one journaled owner-only HEAD helper under the exact `tarasevych.pavlo@gmail.com` Apps Script project. It retained the three newest owner sessions, preserved the other user, freed 20 slots, and touched no Gmail/OAuth/Telegram mail state. HEAD was immediately restored and hash-verified as exact immutable v32; stable deployment stayed v32.
- Built explicit one-use capacity recovery on an isolated branch. It keeps two existing sessions plus the new one, never silently evicts another user, does not replay the consumed launch claim, and exposes an honest Ukrainian recovery screen.
- Verification: 141/141 targeted tests; 363/363 ordinary functional tests; rendered Chrome 17/17 across desktop/mobile with zero application errors.
- Release boundary: the implementation branch is based on unreleased product v40. Preserve it, then transplant only the session hotfix onto exact product v38 before creating immutable v33.
