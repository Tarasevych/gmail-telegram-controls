...
## F-04 trustworthy analysis evidence / Доказ F-04 достовірного аналізу

- **Status / Статус:** source/tests/docs are `VERIFIED`; overall F-04 remains `PARTIAL` because native populated-reader and current-production acceptance are `UNVERIFIED` / source/tests/docs — `VERIFIED`; загальний F-04 лишається `PARTIAL`, бо native populated-reader і current-production acceptance — `UNVERIFIED`.
- **Source commit / Коміт джерела:** `6b7054331f653d4503b192d3d64f5780e19a837e`.
- **Normal merge / Звичайне злиття:** PR [#98](https://github.com/Tarasevych/gmail-telegram-controls/pull/98), merge `c8cfb942e1764d32eb072f55f51e2007133de2d2`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `c8cfb942e1764d32eb072f55f51e2007133de2d2`.
- **Root cause / Першопричина:** automated analysis was expanded without an independent per-account off switch; a blanket short-text threshold discarded some meaningful replies while common mobile signatures remained; unsupported action/date/amount/risk claims could survive normalization; generated next action appeared inside the persisted-decision field / automated analysis був розгорнутий без незалежного per-account вимикача; blanket short-text threshold відкидав частину meaningful replies, але лишав mobile signatures; unsupported action/date/amount/risk claims могли пережити normalization; generated next action виглядав як persisted decision.
- **Implemented / Реалізовано:** account-scoped `collapsed|expanded|hidden` preference with default collapse; explicit no-substantive-summary result without translation for empty/signature-only content; short meaningful reply preservation; exact source-fragment gates; explicit proposal acceptance; accessible persisted triage undo.
- **Tests / Тести:** focused server/UI contracts `246/246`; full Apps Script suite `642/642`; GitHub checks `8/8`; bilingual documentation `95` pairs, knowledge hub, verification-report, release-state, and diff gates passed.
- **Records / Записи:** `GT-065`, `B1-45`, `RCA-019`, `VR-038`.
- **Primary references / Первинні джерела:** [Apps Script Language service](https://developers.google.com/apps-script/reference/language), [Gmail API REST reference](https://developers.google.com/workspace/gmail/api/reference/rest).
- **Visual boundary / Візуальна межа:** local app shell passed desktop `1280×720` and mobile `390×844` without horizontal overflow; owner-private screenshots were retained outside Git because preview identifiers are sensitive. The fail-closed synthetic account mismatch prevented honest populated-reader visual acceptance / local app shell пройшов desktop `1280×720` і mobile `390×844` без horizontal overflow; owner-private screenshots збережено поза Git через sensitive preview identifiers. Fail-closed synthetic account mismatch не дозволив чесний populated-reader visual acceptance.
- **Boundary / Межа:** no external AI endpoint, real Gmail read or mutation, OAuth, Telegram runtime mutation, staging, production promotion, immutable release, token read, or secret-property read / без зовнішнього AI endpoint, читання чи змін real Gmail, OAuth, змін Telegram runtime, staging, production promotion, immutable release, читання token або secret properties.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною.
...
## F-02 message capabilities evidence / Доказ F-02 можливостей дій із листом

- **Status / Статус:** source/tests/docs are `VERIFIED`; overall F-02 remains `PARTIAL` because native Telegram WebView popup/deep-link and browser-settings acceptance is `UNVERIFIED` / source/tests/docs `VERIFIED`; загальний F-02 залишається `PARTIAL`, бо native Telegram WebView popup/deep-link і browser-settings acceptance — `UNVERIFIED`.
- **Source commit / Коміт джерела:** `f6516df305083f2d14f62a8846cf34d181187fd9`.
- **Normal merge / Звичайне злиття:** PR [#96](https://github.com/Tarasevych/gmail-telegram-controls/pull/96), merge `d528cd6031dd0f8551b0e7389c30aec73c591af0`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `d528cd6031dd0f8551b0e7389c30aec73c591af0`.
- **Implemented / Реалізовано:** one primary message-action surface per viewport; compact desktop Reply/Forward/More and one mobile action bar; progressive secondary actions; exact stable `accountId` lookup before account-email handoff; `authuser` URLs without `/u/0`; fail-closed ambiguous identity; eleven-section settings capability hub; honest boundaries for raw MIME, phishing report, translation, print, and feedback.
- **Defect caught by the gate / Дефект, знайдений gate:** the first canonical-state refinement could fall back to the active account when a thread carried an unknown stable account ID; the focused test failed, and the resolver was corrected so an explicit ID permits only an exact ID match while email fallback is allowed only when no ID exists.
- **Tests / Тести:** focused message-capability contract `6/6`; focused Mail App group `98/98`; full Apps Script suite `635/635`; GitHub checks `8/8`; bilingual documentation, knowledge hub, verification report, release-state, diff, and changed-file sensitive-signature gates passed.
- **Records / Записи:** `GT-064`, `B1-44`, `RCA-017`, `VR-036`.
- **Primary references / Первинні джерела:** [Gmail users.settings](https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.settings), [Gmail users.messages.get](https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.messages/get), [Gmail users.threads.modify](https://developers.google.com/workspace/gmail/api/reference/rest/v1/users.threads.modify).
- **Boundary / Межа:** no new RPC, OAuth scope, raw MIME transfer, Gmail/Telegram mutation, staging, production promotion, release-helper mutation, token read, or secret-property read / без нового RPC, OAuth scope, raw MIME transfer, змін Gmail/Telegram, staging, production promotion, release-helper mutation, читання токенів або secret properties.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною.

## F-03 typography-regression evidence / Доказ F-03 typography regression

- **Status / Статус:** source/tests/docs and local desktop/mobile computed-style acceptance are `VERIFIED`; overall F-03 remains `PARTIAL` because current-production same-scale comparison, populated real-thread/list acceptance, and native browser zoom are `UNVERIFIED` / source/tests/docs і local desktop/mobile computed-style acceptance — `VERIFIED`; загальний F-03 лишається `PARTIAL`, бо current-production same-scale comparison, populated real-thread/list acceptance і native browser zoom — `UNVERIFIED`.
- **Source commit / Коміт джерела:** `e1d5c1e9e6be07e0897ccacd5f84c88a69a22637`.
- **Normal merge / Звичайне злиття:** PR [#97](https://github.com/Tarasevych/gmail-telegram-controls/pull/97), merge `af3b32ee13386f5f26a9d6f244bc330e70bf41d0`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `af3b32ee13386f5f26a9d6f244bc330e70bf41d0`.
- **Root cause / Першопричина:** the first Gmail-compatible type-scale correction did not cover legacy 10 px compose account/save status, 11 px settings/account metadata, or nowrap account identity labels / перше Gmail-compatible type-scale виправлення не охопило legacy 10 px compose account/save status, 11 px settings/account metadata і nowrap account identity labels.
- **Implemented / Реалізовано:** 12 px readable secondary scale; 14 px reading/compose scale retained; wrap-safe account identities; no blanket bold; no remote font; header flex shrink/wrap boundary.
- **Tests / Тести:** focused typography `5/5`; full Apps Script suite `640/640`; validator unit tests `6/6`; GitHub checks `8/8`; bilingual documentation `94` pairs, knowledge hub, release-state, verification-report, and diff gates passed.
- **Records / Записи:** existing `GT-032`, existing `B1-25`, new `RCA-018`, new `VR-037`; the V3 plan reference to `GT-035` was rejected because the factual registry assigns `GT-035` to draft recovery / existing `GT-032`, existing `B1-25`, new `RCA-018`, new `VR-037`; посилання V3 plan на `GT-035` відхилено, бо factual registry відносить `GT-035` до recovery чернеток.
- **Visual boundary / Візуальна межа:** owner-private synthetic screenshots were inspected but not published because the preview exposed account identifiers; the public report contains content-free computed values only / owner-private synthetic screenshots перевірено, але не опубліковано через account identifiers; публічний звіт містить лише content-free computed values.
- **Boundary / Межа:** no Gmail/Telegram mutation, OAuth, staging, production promotion, immutable release, token read, or secret-property read / без змін Gmail/Telegram, OAuth, staging, production promotion, immutable release, читання токенів або secret properties.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною.

## F-05 reading-progress evidence / Доказ F-05 прогресу читання

- **Status / Статус:** source/tests/docs and local synthetic desktop/mobile geometry are `VERIFIED`; overall F-05 remains `PARTIAL` because native Telegram Desktop/mobile and current-production acceptance are `UNVERIFIED` / source/tests/docs і local synthetic desktop/mobile geometry — `VERIFIED`; загальний F-05 лишається `PARTIAL`, бо native Telegram Desktop/mobile і current-production acceptance — `UNVERIFIED`.
- **Source commit / Коміт джерела:** `31daf927c5d291f4d530ceb4e5532c5955f012c6`.
- **Normal merge / Звичайне злиття:** PR [#99](https://github.com/Tarasevych/gmail-telegram-controls/pull/99), merge `159d9c7f177f0e42b414f95efdb9d3b5ea3a4c86`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `159d9c7f177f0e42b414f95efdb9d3b5ea3a4c86`.
- **Root cause / Першопричина:** short non-scrollable content returned false `100%`; copy described scroll position as content being read; an empty `0%` resume control could remain visible; manual resume ignored reduced motion; and a delayed callback retained no exact thread/Gmail-connection identity / short non-scrollable content повертав false `100%`; text називав scroll position прочитаним змістом; порожній `0%` resume control міг лишатися видимим; manual resume ігнорував reduced motion; delayed callback не утримував exact thread/Gmail-connection identity.
- **Implemented / Реалізовано:** measurable geometry snapshot; honest scroll-position semantics; compact omission without a draft or meaningful position; exact `threadId + connectionId` debounce guard; reduced-motion-aware user resume; existing anchor/offset/bottom/focus restoration retained without timer-derived progress or background auto-scroll.
- **Tests / Тести:** focused reader contracts `12/12`; full Apps Script suite `646/646`; GitHub checks `8/8`; bilingual documentation `96` pairs, knowledge hub, verification-report, and diff gates passed.
- **Records / Записи:** existing `GT-050`, existing `B1-30`, new `RCA-020`, new `VR-039`; no duplicate GT/B1 was invented.
- **Visual boundary / Візуальна межа:** content-free synthetic desktop `1280×720` and mobile iframe `390×844` both had no horizontal overflow; rail geometry was `720px` desktop and `326×58` mobile. No private mailbox content or identifier was used or retained.
...
## C-03 folder-upload evidence / Доказ C-03 завантаження папок

- **Status / Статус:** source/tests/docs are `VERIFIED`; overall C-03 remains `PARTIAL` because native Telegram WebView folder-picker, mobile/desktop visual acceptance, staging, and production are `UNVERIFIED` / source/tests/docs — `VERIFIED`; загальний C-03 лишається `PARTIAL`, бо native Telegram WebView folder-picker, mobile/desktop visual acceptance, staging і production — `UNVERIFIED`.
- **Source commit / Коміт джерела:** `a33242df9689f6d483825940632df3030663d1a6`.
- **Normal merge / Звичайне злиття:** PR [#120](https://github.com/Tarasevych/gmail-telegram-controls/pull/120), merge `561d5365538de0e7762e6a9862ca75697a03582b`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `561d5365538de0e7762e6a9862ca75697a03582b`.
- **Root cause / Першопричина:** native directory recursion used the current attachment count as its scan cap, while `webkitdirectory` fallback had no whole-selection admission plan; recursive path, duplicate, aggregate-byte, rejection, and batch status were fragmented across individual jobs / native directory recursion використовував current attachment count як scan cap, а `webkitdirectory` fallback не мав whole-selection admission plan; recursive path, duplicate, aggregate-byte, rejection і batch status були розпорошені між окремими jobs.
- **Implemented / Реалізовано:** one bounded `1000`-entry planner before FileReader; normalized recursive relative paths; fail-closed traversal/hidden/service/empty/exact-duplicate rules; aggregate count/byte gate; progressive `40`-row batch summary; per-file retry/cancel, cancel-all, and truthful Drive fallback; existing bounded transfer manager retained.
- **Tests / Тести:** focused folder-upload contracts `9/9`; Mail App contract `93/93`; full Apps Script suite `716/716` in `25.980s`; GitHub checks passed; bilingual documentation `106` pairs, knowledge hub, verification-report, validator-regression, and diff gates passed.
- **Records / Записи:** `GT-074`, `B1-54`, `RCA-030`, `VR-049`.
- **Boundary / Межа:** `100/1000` ordinary attachments are not falsely accepted; oversized batches stop before local reads and offer Drive. No real Gmail message or attachment mutation, OAuth, Telegram runtime mutation, staging, production promotion, immutable release, token read, or secret-property read / `100/1000` ordinary attachments не приймаються неправдиво; oversized batches зупиняються до local reads і пропонують Drive. Без змін real Gmail листів чи вкладень, OAuth, Telegram runtime, staging, production promotion, immutable release, читання token або secret properties.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged; shared quota/T-03 blockers still prohibit safe cumulative staging / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною; shared quota/T-03 blockers досі забороняють safe cumulative staging.
...
- **Source commit / Коміт джерела:** `3420c3919aa09677c2902cfdd4dace494a54a883`.
- **Normal merge / Звичайне злиття:** PR [#101](https://github.com/Tarasevych/gmail-telegram-controls/pull/101), merge `2e85b6173b485501f3eb9ff765fb91fe6c66a78f`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `2e85b6173b485501f3eb9ff765fb91fe6c66a78f`.
- **Root cause / Першопричина:** client compose UI defaulted to a Gmail-saved label, mixed dirty/in-flight/pending states, exposed manual “check saving” during normal autosave, and lacked one bounded same-session terminal retry presentation; server canonical readback and stable-operation idempotency were already correct.
- **Implemented / Реалізовано:** derived `changed/saving/saved/offline/conflict/error` presentation; non-colour canonical acknowledgement mark and time; offline queue; three bounded automatic attempts; exact-operation manual retry only after terminal state; existing local/Gmail conflict preservation; polite live status; no server draft-RPC change.
- **Tests / Тести:** focused compose/draft contracts `104/104`; full Apps Script suite `651/651`; GitHub checks `8/8`; bilingual documentation `97` pairs, knowledge hub, verification-report, and diff gates passed.
- **Records / Записи:** existing `GT-035`, existing `GT-045`, existing `B1-25`, new `RCA-021`, new `VR-040`; no duplicate GT/B1 was invented.
- **Visual boundary / Візуальна межа:** synthetic desktop transitioned `Змінено` to canonical `Збережено` with a `12px`, `30px` status; mobile `390×844` had zero horizontal overflow and no status/account overlap; offline edit remained queued and reached canonical saved state after network recovery. Owner identifiers and screenshots were not published.
- **Boundary / Межа:** no real Gmail draft or message mutation, OAuth, Telegram runtime mutation, staging, production promotion, immutable release, token read, secret-property read, or server draft-RPC change / без змін real Gmail draft чи листів, OAuth, Telegram runtime, staging, production promotion, immutable release, читання token, secret properties або server draft RPC.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged; shared quota/T-03 blockers still prohibit safe cumulative staging / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною; shared quota/T-03 blockers досі забороняють safe cumulative staging.

## C-04 rich-compose evidence / Доказ C-04 rich compose

- **Status / Статус:** source/tests/docs are `VERIFIED`; overall C-04 remains `PARTIAL` because browser-native desktop/mobile acceptance, staging, and production are `UNVERIFIED` or `BLOCKED` / source/tests/docs — `VERIFIED`; загальний C-04 лишається `PARTIAL`, бо browser-native desktop/mobile acceptance, staging і production — `UNVERIFIED` або `BLOCKED`.
- **Source commit / Коміт джерела:** `f790897e8dec4a83e8ab8c7114618109b99b436a`.
- **Evidence commit / Коміт доказів:** `b09c553`.
- **Normal merge / Звичайне злиття:** PR [#122](https://github.com/Tarasevych/gmail-telegram-controls/pull/122), merge `689c401397be8419df60239063ebe831477e96ba`.
- **Remote parity / Паритет remote:** GitHub `main` = GitLab `main` = `689c401397be8419df60239063ebe831477e96ba`.
- **Root cause / Першопричина:** recipients remained raw strings without token-level controls, rich paste was reduced to plain text, advanced formatting was permanently exposed, and table semantics were not regenerated after sanitized rerender / recipients лишалися raw strings без token-level controls, rich paste зводився до plain text, advanced formatting була постійно відкрита, а table semantics не відновлювалися після sanitized rerender.
- **Implemented / Реалізовано:** one string-compatible To/CC/BCC chip adapter without a parallel draft model; quoted, Unicode, duplicate, invalid, and 100-recipient handling; bounded internal scrolling; keyboard add/remove/navigation; sanitizer-only rich paste; progressive secondary formatting; retained selection; regenerated table ARIA, Tab traversal, and whole-table deletion / один string-compatible To/CC/BCC chip adapter без паралельної draft-моделі; обробка quoted, Unicode, duplicate, invalid і 100 recipients; bounded internal scrolling; keyboard add/remove/navigation; лише sanitized rich paste; progressive secondary formatting; збереження selection; regenerated table ARIA, Tab traversal і whole-table delete.
- **Tests / Тести:** focused C-04 contracts `5/5`; affected compose matrix `116/116`; MailClient `153/153`; full Apps Script suite `721/721` in `25.457s`; bilingual documentation `107` pairs, knowledge hub, verification reports, validator regression, diff, and GitHub parity/validation checks passed.
- **Records / Записи:** `GT-075`, `B1-55`, `RCA-031`, `VR-050`.
- **Visual boundary / Візуальна межа:** the available Chrome and Codex in-app browser bindings exposed no callable automation contract, so browser-native visual acceptance was not fabricated and remains `UNVERIFIED` / доступні Chrome і Codex in-app browser bindings не надали callable automation contract, тому browser-native visual acceptance не вигадувалася й лишається `UNVERIFIED`.
- **Boundary / Межа:** no backend/API/OAuth change, real Gmail read or mutation, Telegram runtime mutation, staging, production promotion, immutable release, token read, or secret-property read / без змін backend/API/OAuth, читання чи змін real Gmail, Telegram runtime mutation, staging, production promotion, immutable release, читання token або secret properties.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical and unchanged; shared quota/T-03 blockers still prohibit safe cumulative staging / production лишається v65, staging `0`, immutable v70 лишається історичною та незмінною; shared quota/T-03 blockers досі забороняють safe cumulative staging.

## V3 task-code coverage reconciliation / Звірка покриття V3 task-code

- **Status / Статус:** knowledge-map reconciliation is `VERIFIED`; overall V3 execution remains `PARTIAL` / knowledge-map reconciliation — `VERIFIED`; загальне виконання V3 лишається `PARTIAL`.
- **Baseline / Початковий стан:** exact-ID scan found `26/31` V3 task codes in active registries; `A-04`, `A-05`, `D-02`, `E-01`, and `E-02` had existing source/test contours but no explicit alias / exact-ID scan знайшов `26/31` V3 task codes; п'ять чинних source/test contours не мали явного alias.
- **Correction / Виправлення:** `B1-56` now maps all `31/31` task IDs to existing `B1/GT/VR` evidence without duplicating product code or rewriting historical reports / `B1-56` тепер маршрутизує всі `31/31` task IDs до чинних доказів без дублювання product code чи переписування історичних звітів.
- **Evidence / Доказ:** product/docs commit `22d3350`; PR [#124](https://github.com/Tarasevych/gmail-telegram-controls/pull/124); normal merge `de881a6450dc997b8c9a79bc16f97b7a0cd83aba`; GitHub `main` = GitLab `main`.
- **Validation / Перевірка:** exact coverage `31/31`; bilingual docs `108` pairs; knowledge hub, verification reports, `3/3` verification-validator tests, diff, and GitHub checks passed.
- **Records / Записи:** `GT-076`, `B1-56`, `RCA-032`, `VR-051`; linked P0 request `REQ-0037`.
- **Boundary / Межа:** alias coverage is not native/runtime DoD. Device-bound offline/fresh shell, version transition, Box runtime, viewport, and pane acceptance keep their existing `PARTIAL/UNVERIFIED/BLOCKED` status / alias coverage не є native/runtime DoD; чинні межі лишаються без підвищення.
- **Release state / Стан релізу:** production remains v65, staging remains `0`, immutable v70 remains historical; shared URL Fetch quota and `T-03` still block release continuation / production лишається v65, staging `0`, immutable v70 історичний; shared URL Fetch quota і `T-03` досі блокують release continuation.
