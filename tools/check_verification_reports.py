...
def main() -> int:
    ...
    if index.get("latest_report") != REPORT_ID or schema.get("schema_version") != 1:
        errors.append("Verification index/schema metadata is invalid")

    serialized = json.dumps({"claims": claims, "manifest": manifest, "index": index, "schema": schema}, ensure_ascii=False)
    for label, pattern in SECRET_PATTERNS.items():
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print(f"Verification report check passed: 245 claims, statuses={dict(status_counts)}, grades={complete_grade_counts}")
    return 0
