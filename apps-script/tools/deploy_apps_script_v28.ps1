...
$ExpectedOldVersion = 27
$ExpectedNewVersion = 28
$Description = 'Telegram Gmail client v28: neuroinclusive Focus View, triage, next action and Resume Rail'
...
$token = $null
$refresh = $null
$releaseMutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV28Release')
...
try {
  ...
  if (-not $releaseMutexHeld) {
    throw 'Another local v28 deployment or migration guard is already running.'
  }
  ...
  try {
    ...
    try {
      Assert-FileSetAndHashes $head $ExpectedCandidateHashes 'Current HEAD'
      $headState = 'candidate_v28'
    } catch {
      throw 'Current HEAD is neither the exact v27 rollback source nor the exact v28 candidate.'
    }
  }

  # Probe repeatedly before POST. If an earlier process created v28 but lost its
  # response, adopt that immutable snapshot; never create a later version on retry.
  $immutableV28 = Get-ImmutableVersionOrNull $base $ExpectedNewVersion 4
  $candidateVersionReady = $null -ne $immutableV28
  if ($candidateVersionReady) {
    Assert-FileSetAndHashes $immutableV28 $ExpectedCandidateHashes "Existing immutable version $ExpectedNewVersion"
  }
  $releaseState = if ($candidateVersionReady) {
    'resume_existing_v28'
  } elseif ($headState -eq 'candidate_v28') {
  ...
  try {
    ...
    if (-not $candidateVersionReady) {
      ...
      # previous ambiguous v28 create into an accidental later version. With one recovery
      ...
      if ($blockingVersions.Count) {
        $immutableV28 = Get-ImmutableVersionOrNull $base $ExpectedNewVersion 6
        if ($null -eq $immutableV28) {
    ...
    if (-not $candidateVersionReady) {
      ...
      Assert-FileSetAndHashes $immutableV28 $ExpectedCandidateHashes "Immutable version $ExpectedNewVersion"
      $candidateVersionReady = $true
    }

    # Once exact immutable v28 exists, never roll it or HEAD back. Any later
    # uncertainty is resumable and the next invocation adopts v28.
    ...
    [pscustomobject]@{
      ok = $true
      mode = if ($adoptedExistingVersion) { 'resume_existing_v28' } else { 'deploy' }
      oldVersion = $ExpectedOldVersion
      newVersion = $ExpectedNewVersion
      deploymentId = $DeploymentId
