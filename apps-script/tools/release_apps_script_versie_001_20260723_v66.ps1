...
$RollbackVersion = 65
$LegacyStagingVersion = 65
$CandidateVersion = 66
$ReleaseDescription = 'Telegram Gmail Versie 1 (2026-07-23): restore SENT+INBOX exactly-once delivery'
$StagingDescription = 'Telegram Gmail Versie 1 (2026-07-23) restore SENT+INBOX exactly-once delivery'
$LegacyStagingDescription = 'Telegram Gmail Versie 1 (2026-07-23) align production client release discovery'
$ProjectRoot = Split-Path -Parent $PSScriptRoot
$SourceMainSha = 'd2da9d9b7393e125b3e6592f6d0fd527381946c0'
$JournalPath = Join-Path $HOME '.codex\recovery\019f7620-bb25-7bd0-a9ea-9034d55103ac-versie-001-20260723-v66-release.json'
...
function Wait-DeploymentVersion([string]$Uri, [int]$ExpectedVersion, [int[]]$AllowedPriorVersions) {
  $maxAttempts = 5
  for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
    $observedVersion = [int](Invoke-GoogleJson GET $Uri).deploymentConfig.versionNumber
    if ($observedVersion -eq $ExpectedVersion) { return }
    if ($observedVersion -notin $AllowedPriorVersions) {
      throw "Deployment reconciliation observed unsupported v$observedVersion."
    }
    if ($attempt -lt $maxAttempts) { Start-Sleep -Seconds 1 }
  }
  throw "Deployment did not converge to v$ExpectedVersion after $maxAttempts bounded reads."
}
...
$mutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierVersie00120260723V66Release')
...
try {
  ...
  Invoke-GoogleJson PUT $stableUri @{deploymentConfig=@{scriptId=$ScriptId;versionNumber=$RollbackVersion;
    manifestFileName='appsscript';description='Rollback to verified Telegram Gmail Versie 1 Apps Script v65'}} | Out-Null
  Set-Head $base $rollbackContent $ExpectedRollbackHashes "Rolled back v$RollbackVersion HEAD"
  Wait-DeploymentVersion $stableUri $RollbackVersion @($CandidateVersion)
