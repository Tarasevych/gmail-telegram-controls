...
$CandidateVersion = 37
$ReleaseDescription = 'Telegram Gmail product v45: gentle milestones and exact-session logout'
$StagingDescription = 'Telegram Gmail product v45 guarded staging'
$LegacyStagingDescription = 'Telegram Gmail product v43 isolated staging'
$ProjectRoot = Split-Path -Parent $PSScriptRoot
$JournalPath = Join-Path $HOME '.codex\recovery\019f5d65-8209-7a00-b915-4a522dbcb612-v37-product-v45-release.json'
...
Assert-Hashes $candidate $ExpectedCandidateHashes 'Local product-v45 candidate'

$mutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV37ProductV45Release')
$held = $false; $script:AccessToken = $null
try {
  $held = $mutex.WaitOne(0)
  if (-not $held) { throw 'Another product-v45 release process is active.' }
  ...
  if ($StageOnly) {
    ...
    if ($legacyStaging.Count) { throw 'Clean the exact product-v43 staging deployment before staging product-v45.' }
    try {
      if (-not $immutable) {
        ...
        if ($headState -ne "candidate_v$CandidateVersion") {
          Set-Head $base $candidate $ExpectedCandidateHashes 'Uploaded product-v45 HEAD'
      ...
      if ($staging.Count -ne 1) { throw 'Product-v45 staging was not verified exactly once.' }
    } finally {
  ...
  if ($CleanupStaging) {
    if ($stableVersion -ne $CandidateVersion -or -not $immutable) { throw 'Cleanup requires stable exact product-v45.' }
  ...
  if ($stableVersion -ne $CandidateVersion -or -not $immutable) { throw 'Rollback requires stable exact product-v45.' }
