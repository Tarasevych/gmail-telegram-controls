...
$ExpectedNewVersion = 32
$ReleaseDescription = 'Telegram Gmail product v38: mobile settings reachability'
$StagingDescription = 'Telegram Gmail product v38 immutable WebView staging'
$ProjectRoot = Split-Path -Parent $PSScriptRoot
$JournalPath = Join-Path $HOME '.codex\recovery\019f5d65-8209-7a00-b915-4a522dbcb612-v32-product-v38-release.json'
...
function Read-ReleaseJournal {
  ...
  try {
    ...
    throw 'The local v32/product-v38 release journal is corrupt; refusing mutations.'
  }
...
Assert-FileSetAndHashes $candidate $ExpectedCandidateHashes 'Local product-v38 candidate'

$script:AccessToken = $null
$clasp = $null
$token = $null
$refresh = $null
$mutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV32ProductV38Release')
$mutexHeld = $false
try {
  try { $mutexHeld = $mutex.WaitOne(0) } catch [Threading.AbandonedMutexException] { $mutexHeld = $true }
  if (-not $mutexHeld) { throw 'Another local Apps Script v32/product-v38 release operation is running.' }
  ...
  if ($null -eq $previousImmutable) {
    throw "Required immutable product-v37 version $ExpectedPreviousVersion is absent."
  }
  ...
  if ($StageOnly) {
    ...
    try {
      $rollbackHashes = if ($headState -eq 'previous_candidate_v31') { $ExpectedPreviousCandidateHashes } else { $ExpectedOldHashes }
      if ($headState -ne 'candidate_product_v38') {
        $headWasMutated = $true
        $null = Set-HeadAndAssertHashes $base $candidate $ExpectedCandidateHashes 'Uploaded product-v38 HEAD'
      }
      ...
      if (-not $immutableReady) {
        ...
        if ($inventory.Count) {
          ...
          if ($null -eq $immutable) { throw 'Version inventory contains v32 but its content is unavailable.' }
        } else {
          if ((Get-JournalState $journal) -eq 'version_create_reserved') {
            throw 'A prior immutable v32 create has an unresolved outcome; refusing another versions.create.'
          }
          ...
          if ($null -eq $immutable) {
            ...
            throw 'Apps Script did not expose immutable v32 after create.'
          }
        }
        Assert-FileSetAndHashes $immutable $ExpectedCandidateHashes "Immutable version $ExpectedNewVersion"
        $immutableReady = $true
      ...
      if (-not $immutableReady -and $headWasMutated) {
        ...
        throw "Staging failed before immutable v32 verification and HEAD rollback completed: $($original.Exception.Message)"
      }
      throw "Staging paused after immutable v32 was verified; never create another version automatically. Original error: $($original.Exception.Message)"
    }
  }

  if ($Promote) {
    if ($null -eq $immutable) { throw 'Cannot promote: immutable v32 is absent.' }
    ...
    if ($stableVersion -eq $ExpectedOldVersion) {
      ...
      if ([int]$stableDeployment.deploymentConfig.versionNumber -ne $ExpectedNewVersion) {
        ...
        throw 'Stable deployment did not advance to v32.'
      }
    }
  ...
  if ($CleanupStaging) {
    if ($stableVersion -ne $ExpectedNewVersion) {
      throw 'CleanupStaging requires the stable deployment to be verified on v32.'
    }
