...
$ExpectedNewVersion = 30
$ReleaseDescription = 'Telegram Gmail product v36: integrated neuroinclusive P1'
...
function Read-ReleaseJournal {
  ...
  try {
    ...
    throw 'The local v30/product-v36 release journal is corrupt; refusing mutations.'
  }
...
Assert-FileSetAndHashes $candidate $ExpectedCandidateHashes 'Local product-v36 candidate'

$script:AccessToken = $null
$clasp = $null
$token = $null
$refresh = $null
$mutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV30ProductV36Release')
$mutexHeld = $false
try {
  try { $mutexHeld = $mutex.WaitOne(0) } catch [Threading.AbandonedMutexException] { $mutexHeld = $true }
  if (-not $mutexHeld) { throw 'Another local Apps Script v30/product-v36 release operation is running.' }
  ...
  try {
    ...
    $headState = 'candidate_product_v36'
  }

  $staging = @(Get-ExactStagingDeployments $base)
  if ($staging.Count -gt 1 -and -not $CleanupStaging) {
    throw 'More than one exact product-v36 staging deployment exists.'
  }
  ...
  if ($StageOnly) {
    ...
    try {
      if ($headState -eq 'stable_v29') {
        $headWasMutated = $true
        $null = Set-HeadAndAssertHashes $base $candidate $ExpectedCandidateHashes 'Uploaded product-v36 HEAD'
      }
      ...
      if (-not $immutableReady) {
        ...
        if ($inventory.Count) {
          ...
          if ($null -eq $immutable) { throw 'Version inventory contains v30 but its content is unavailable.' }
        } else {
          if ((Get-JournalState $journal) -eq 'version_create_reserved') {
            throw 'A prior immutable v30 create has an unresolved outcome; refusing another versions.create.'
          }
          ...
          if ($null -eq $immutable) {
            ...
            throw 'Apps Script did not expose immutable v30 after create.'
          }
        }
        Assert-FileSetAndHashes $immutable $ExpectedCandidateHashes "Immutable version $ExpectedNewVersion"
        $immutableReady = $true
      ...
      if (-not $immutableReady -and $headWasMutated) {
        $null = Set-HeadAndAssertHashes $base @{ scriptId = $ScriptId; files = $oldImmutable.files } $ExpectedOldHashes 'Rolled-back HEAD'
        throw "Staging failed before immutable v30 verification and HEAD rollback completed: $($original.Exception.Message)"
      }
      throw "Staging paused after immutable v30 was verified; never create another version automatically. Original error: $($original.Exception.Message)"
    }
  }

  if ($Promote) {
    if ($null -eq $immutable) { throw 'Cannot promote: immutable v30 is absent.' }
    ...
    if ($stableVersion -eq $ExpectedOldVersion) {
      ...
      if ([int]$stableDeployment.deploymentConfig.versionNumber -ne $ExpectedNewVersion) {
        ...
        throw 'Stable deployment did not advance to v30.'
      }
    }
  ...
  if ($CleanupStaging) {
    if ($stableVersion -ne $ExpectedNewVersion) {
      throw 'CleanupStaging requires the stable deployment to be verified on v30.'
    }
