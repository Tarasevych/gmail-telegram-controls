...
$ExpectedOldVersion = 33
$ExpectedPreviousVersion = 32
$ExpectedNewVersion = 34
$ReleaseDescription = 'Telegram Gmail product v38.2: automatic same-user session compaction'
$StagingDescription = 'Telegram Gmail product v38.2 session compaction staging'
$ProjectRoot = Split-Path -Parent $PSScriptRoot
$JournalPath = Join-Path $HOME '.codex\recovery\019f5d65-8209-7a00-b915-4a522dbcb612-v34-product-v38-session-compaction-release.json'
...
function Read-Journal {
  ...
  if ([string]$j.scriptId -ne $ScriptId -or [int]$j.version -ne $ExpectedNewVersion -or
      [string]$j.candidateMailAppHash -ne $ExpectedCandidateHashes.MailApp -or
      [string]$j.candidateMailClientHash -ne $ExpectedCandidateHashes.MailClient) {
...
function Write-Journal([string]$State, [string]$StagingId = '') {
  [ordered]@{scriptId=$ScriptId;version=$ExpectedNewVersion;candidateMailAppHash=$ExpectedCandidateHashes.MailApp;
    candidateMailClientHash=$ExpectedCandidateHashes.MailClient;
...
$mutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV34ProductV38SessionCompactionRelease')
$mutexHeld = $false
$script:AccessToken = $null
try {
  $mutexHeld = $mutex.WaitOne(0)
  if (-not $mutexHeld) { throw 'Another v34 release process is active.' }
  ...
  Assert-Hashes (Invoke-GoogleJson GET "$base/content?versionNumber=$ExpectedOldVersion") $ExpectedOldHashes 'Immutable v33'
  Assert-Hashes (Invoke-GoogleJson GET "$base/content?versionNumber=$ExpectedPreviousVersion") $ExpectedPreviousHashes 'Immutable v32'
  ...
  if ($immutable) { Assert-Hashes $immutable $ExpectedCandidateHashes 'Immutable v34' }
  $head = Invoke-GoogleJson GET "$base/content"
  $headState = 'unknown'
  try { Assert-Hashes $head $ExpectedOldHashes 'HEAD'; $headState='stable_v33' }
  catch { Assert-Hashes $head $ExpectedCandidateHashes 'HEAD'; $headState='candidate_v34' }
  ...
  if ($staging.Count -gt 1) { throw 'Multiple v34 staging deployments exist.' }
  ...
  if ($StageOnly) {
    if ($stableVersion -ne 33) { throw 'StageOnly requires stable v33.' }
    if (-not $immutable) {
      ...
      $immutable = Get-Immutable $base $ExpectedNewVersion
      if (-not $immutable) { throw 'Immutable v34 was not observed after create.' }
      Assert-Hashes $immutable $ExpectedCandidateHashes 'Created immutable v34'
    ...
    if ($staging.Count -ne 1) { throw 'Staging v34 was not verified exactly once.' }
    ...
    [ordered]@{ok=$true;mode='stage';stableVersion=$ExpectedOldVersion;immutableVersion=$ExpectedNewVersion;stagingDeploymentId=[string]$staging[0].deploymentId;
  ...
  if ($Promote) {
    if (-not $immutable -or $staging.Count -ne 1 -or -not $journal -or [string]$journal.state -ne 'staging_verified') {
      throw 'Cannot promote without exact immutable v34, one staging deployment, and acceptance journal.'
    }
    if ($stableVersion -eq $ExpectedOldVersion) {
      Invoke-GoogleJson PUT $stableUri @{deploymentConfig=@{scriptId=$ScriptId;versionNumber=$ExpectedNewVersion;manifestFileName='appsscript';description=$ReleaseDescription}} | Out-Null
    }
    $stable = Invoke-GoogleJson GET $stableUri
    if ([int]$stable.deploymentConfig.versionNumber -ne 34) { throw 'Stable did not advance to v34.' }
    Write-Journal 'promoted' ([string]$staging[0].deploymentId)
    [ordered]@{ok=$true;mode='promote';oldVersion=$ExpectedOldVersion;newVersion=$ExpectedNewVersion;deploymentId=$StableDeploymentId} | ConvertTo-Json
    return
  }

  if ($stableVersion -ne 34 -or $staging.Count -ne 1) { throw 'Cleanup requires stable v34 and one staging deployment.' }
