...
$ExpectedOldVersion = 25
$ExpectedNewVersion = 26
$Description = 'Telegram Gmail client v26: isolated multi-user Gmail, independent cloud-source accounts and exact account routing'
...
try {
  ...
  try {
    ...
    try {
      Assert-FileSetAndHashes $head $ExpectedCandidateHashes 'Current HEAD'
      $headState = 'candidate_v26'
    } catch {
      throw 'Current HEAD is neither the exact v25 rollback source nor the exact v26 candidate.'
    }
  }

  # Probe repeatedly before POST. If an earlier process created v26 but lost its
  ...
  $releaseState = if ($candidateVersionReady) {
    'resume_existing_v26'
  } elseif ($headState -eq 'candidate_v26') {
  ...
  try {
    ...
    if (-not $candidateVersionReady) {
      ...
      # previous ambiguous v26 create into an accidental later version. With one recovery
    ...
    # Once exact immutable v26 exists, never roll it or HEAD back. Any later
    # uncertainty is resumable and the next invocation adopts v26.
    ...
    [pscustomobject]@{
      ok = $true
      mode = if ($adoptedExistingVersion) { 'resume_existing_v26' } else { 'deploy' }
      oldVersion = $ExpectedOldVersion
      newVersion = $ExpectedNewVersion
      deploymentId = $DeploymentId
