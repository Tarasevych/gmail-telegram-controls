...
$ExpectedOldVersion = 28
$ExpectedNewVersion = 29
$Description = 'Telegram Gmail client v29: energy-aware Focus sessions and reminder preferences'
...
$token = $null
$refresh = $null
$releaseMutex = [Threading.Mutex]::new($false, 'Local\TarasevychGmailNotifierAppsScriptV29Release')
...
try {
  ...
  if (-not $releaseMutexHeld) {
    throw 'Another local v29 deployment or migration guard is already running.'
  }
  ...
  # Probe repeatedly before POST. If an earlier process created v29 but lost its
  # response, adopt that immutable snapshot; never create a later version on retry.
  $immutableV29 = Get-ImmutableVersionOrNull $base $ExpectedNewVersion 4
  $candidateVersionReady = $null -ne $immutableV29
  if ($candidateVersionReady) {
    Assert-FileSetAndHashes $immutableV29 $ExpectedCandidateHashes "Existing immutable version $ExpectedNewVersion"
  }
  $releaseState = if ($candidateVersionReady) {
    'resume_existing_v29'
  } elseif ($headState -eq 'candidate_v29') {
  ...
  try {
    ...
    if (-not $candidateVersionReady) {
      ...
      # previous ambiguous v29 create into an accidental later version. With one recovery
      ...
      if ($blockingVersions.Count) {
        $immutableV29 = Get-ImmutableVersionOrNull $base $ExpectedNewVersion 6
        if ($null -eq $immutableV29) {
    ...
    if (-not $candidateVersionReady) {
      ...
      Assert-FileSetAndHashes $immutableV29 $ExpectedCandidateHashes "Immutable version $ExpectedNewVersion"
      $candidateVersionReady = $true
    }

    # Once exact immutable v29 exists, never roll it or HEAD back. Any later
    # uncertainty is resumable and the next invocation adopts v29.
    ...
    [pscustomobject]@{
      ok = $true
      mode = if ($adoptedExistingVersion) { 'resume_existing_v29' } else { 'deploy' }
      oldVersion = $ExpectedOldVersion
      newVersion = $ExpectedNewVersion
      deploymentId = $DeploymentId
