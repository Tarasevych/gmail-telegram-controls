...
test('compose attachment sources use the exact Drive, Box and publicHttps RPC contract without HTML bytes', () => {
  ...
  assert.match(sourceUi, /function classifyPublicSourceInput\(value\)[\s\S]*kind:\s*"google_search"[\s\S]*kind:\s*"ambiguous_wrapper"/);
  assert.match(uiSource, /classification\.kind === "google_search" \|\| classification\.kind === "ambiguous_wrapper"[\s\S]*composePublicInsertLink\.focus\(\)/);
  assert.match(uiSource, /маєте право використовувати й надсилати/);
...
test('pending draft save retries the identical operation and baselines only after canonical Gmail readback', async () => {
  ...
  const context = vm.createContext({
    ...
    p0PersistComposeRecovery: async () => true,
    p0ClearComposeRecoveryKey() {},
    renderCompose() {},
    acknowledgeComposeOperation() {},
    showSnackbar() {},
    safeText: (value, fallback = '') => value == null || value === '' ? String(fallback || '') : String(value),
    composeRestartRecoveryIsRequired: () => false,
    composeRestartRecoveryIsDeferred: () => false,
