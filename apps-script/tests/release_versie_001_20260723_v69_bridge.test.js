...
const bridge = fs.readFileSync(
  path.join(repo, 'versie-001-staging-acceptance-20260723-v69.html'),
  'utf8',
);
const menu = fs.readFileSync(path.join(repo, 'tools', 'update_bot_menu_versie_001.py'), 'utf8');
const stagingBackend = 'https://script.google.com/macros/s/AKfycbzJLq_oePyDdSpYXv_he_MY3-X8xawBfSfBEh0BU084D8Bt5cwhNvByqcckRSoONd7j/exec';
const stagingLauncher = 'https://tarasevych.github.io/gmail-telegram-controls/versie-001-staging-acceptance-20260723-v69.html';
const productionLauncher = 'https://tarasevych.github.io/gmail-telegram-controls/?v=20260715-5&action=mailbox';

test('v69 staging bridge targets only the exact immutable staging deployment', () => {
  assert.ok(bridge.includes(stagingBackend));
  assert.match(bridge, /Gmail Versie 1 staging v69/);
  ...
  assert.doesNotMatch(bridge, /v65|v68/);
});

test('owner menu updater separates exact v69 staging from unchanged production', () => {
  assert.ok(menu.includes('STAGING_URL = "' + stagingLauncher + '"'));
  assert.ok(menu.includes('PRODUCTION_URL = "' + productionLauncher + '"'));
  assert.ok(menu.includes('"🧪 Пошта · Versie 1 v69"'));
  assert.ok(menu.includes('"📬 Пошта · Versie 1"'));
  assert.doesNotMatch(menu, /versie-001-staging-acceptance-20260723-v68\.html/);
});
