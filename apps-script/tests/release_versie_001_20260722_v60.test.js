...
const helperPath = path.join(root, 'tools', 'release_apps_script_versie_001_20260722_v60.ps1');
...
test('v60 helper pins exact v57 rollback, v59 history, and current source', () => {
  assert.match(helper, /\$SourceMainSha\s*=\s*'4440cc938c29dad12d7b1e8575fec17994acbd8f'/);
  assert.match(helper, /\$RollbackVersion\s*=\s*57\b/);
  assert.match(helper, /\$LegacyStagingVersion\s*=\s*59\b/);
  assert.match(helper, /\$CandidateVersion\s*=\s*60\b/);
  ...
  for (const [name, hash] of Object.entries(expectedLegacy)) {
    assert.ok(helper.includes(hash), 'missing immutable v59 historical hash for ' + name);
  }
  for (const [name, hash] of Object.entries(expectedCandidate)) {
    assert.ok(helper.includes(hash), 'missing v60 candidate hash for ' + name);
    assert.equal(normalizedFileHash(name), hash);
  }
});

test('v60 helper fails closed against replay, parallel staging, and stale promotion', () => {
