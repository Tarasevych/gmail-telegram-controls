...
const stagingLauncher = 'https://tarasevych.github.io/gmail-telegram-controls/versie-001-staging-acceptance-20260723-v68.html';
...
test('owner menu updater separates exact v68 staging from unchanged production', () => {
  assert.ok(menu.includes('STAGING_URL = "' + stagingLauncher + '"'));
  assert.ok(menu.includes('PRODUCTION_URL = "' + productionLauncher + '"'));
  assert.ok(menu.includes('"🧪 Пошта · Versie 1 v68"'));
  assert.ok(menu.includes('"📬 Пошта · Versie 1"'));
  assert.doesNotMatch(menu, /versie-001-staging-acceptance-20260723-v67\.html/);
});
