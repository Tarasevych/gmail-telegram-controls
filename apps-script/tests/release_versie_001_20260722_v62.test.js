...
const helperPath = path.join(root, 'tools', 'release_apps_script_versie_001_20260722_v62.ps1');
...
test('v62 helper pins exact v57 rollback, v61 history, and immutable v62 source', () => {
  ...
  assert.match(helper, /\$CandidateVersion\s*=\s*62\b/);
  ...
  for (const [name, hash] of Object.entries(expectedLegacy)) {
    assert.ok(helper.includes(hash), 'missing immutable v61 historical hash for ' + name);
  }
...
test('v62 helper fails closed against replay, parallel staging, and stale promotion', () => {
