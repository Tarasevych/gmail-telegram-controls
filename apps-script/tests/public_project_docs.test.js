...
test('Versie 1 public project guide has synchronized Ukrainian and English control files', () => {
  ...
  for (const relativePath of pairs) {
    ...
    if (relativePath === 'ROADMAP.md' || relativePath === 'ISSUES.md') {
      assert.match(uk, /2026-07-21/);
      assert.match(en, /2026-07-21/);
    } else {
...
test('problem registers keep identical IDs and public docs contain no credential material', () => {
  ...
  const ids = Array.from(uk.matchAll(/^\| (GT-\d{3}) \|/gm), (match) => match[1]);
  assert.deepEqual(ids, Array.from(en.matchAll(/^\| (GT-\d{3}) \|/gm), (match) => match[1]));
  assert.deepEqual(ids, Array.from({ length: ids.length }, (_, index) =>
    `GT-${String(index + 1).padStart(3, '0')}`));
  assert.ok(ids.includes('GT-023'));
