...
test('Build 1 public project guide has synchronized Ukrainian and English control files', () => {
  const pairs = ['README.md', 'VERSIONING.md', 'PROJECT.md', 'ROADMAP.md', 'ISSUES.md', 'releases/BUILD-001-2026-07-19.md'];
  for (const relativePath of pairs) {
    const uk = read(path.join('docs', 'uk', relativePath));
    const en = read(path.join('docs', 'en', relativePath));
    assert.match(uk, /Build 1/);
    assert.match(en, /Build 1/);
