...
test('Versie 1 helper pins stable v41 rollback/staging and immutable v42 candidate', () => {
  assert.match(helper, /\$RollbackVersion = 41/);
  assert.match(helper, /\$LegacyStagingVersion = 41/);
  assert.match(helper, /\$CandidateVersion = 42/);
  assert.match(helper, /Versie 1 \(2026-07-20\): chat-native Google account controls/);
  assert.match(helper, /Telegram Gmail Versie 1 \(2026-07-20\) credentialless OAuth relay staging/);
  assert.match(helper, /versie-001-20260720-v42-release\.json/);
  assert.match(helper, /TarasevychGmailNotifierVersie00120260720V42Release/);
