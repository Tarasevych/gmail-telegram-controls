...
const expected = {
  Code: '274d293e6d99421206e55737f46dbb57e333ec22eb3729dba32e9e8d49ea2755',
...
test('Versie 1 helper pins stable v50 rollback and immutable v54 maintenance candidate', () => {
  assert.match(helper, /\$RollbackVersion = 50/);
  assert.match(helper, /\$LegacyStagingVersion = 53/);
  assert.match(helper, /\$CandidateVersion = 54/);
  assert.match(helper, /Versie 1 \(2026-07-20\): staging-only exact Spam trace maintenance/);
  ...
  assert.match(helper, /\$ExpectedLegacyStagingHashes = @\{[\s\S]*Code='55042fcf036073673709720b121bf184506768de1d2356d0e4aff10a294d27b2'/);
  ...
  assert.match(helper, /versie-001-20260720-v54-release\.json/);
  assert.match(helper, /TarasevychGmailNotifierVersie00120260720V54Release/);
  assert.match(helper, /Invoke-GoogleJson DELETE .*legacyStaging/);
});

test('Versie 1 v54 keeps retention safety and guards exact staging Spam maintenance', () => {
  ...
  assert.match(code, /GMAIL_NOTIFICATION_RUNTIME_CANDIDATE_ = 'v54'/);
  ...
  assert.match(code, /function gmailRuntimeMoveCurrentTraceSpamToInbox_\(traceToken\)/);
  assert.match(code, /traceAction === 'spam_to_inbox'/);
