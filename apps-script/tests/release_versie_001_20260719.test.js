...
const expected = {
  Code: 'd6232875d45ce25522c0a1c694700cb2c9fbf584c722673bf23fbb05c378c6a2',
...
test('Versie 1 helper pins stable v42 rollback and immutable v48 candidate', () => {
  assert.match(helper, /\$RollbackVersion = 42/);
  assert.match(helper, /\$LegacyStagingVersion = 47/);
  assert.match(helper, /\$CandidateVersion = 48/);
  assert.match(helper, /Versie 1 \(2026-07-20\): independent realtime lane lease/);
  ...
  assert.match(helper, /\$ExpectedLegacyStagingHashes = @\{[\s\S]*Code='2a4803d555c2590882b898c7e51088b4061f7fa08d239d72619cf5f8664750f7'/);
  ...
  assert.match(helper, /versie-001-20260720-v48-release\.json/);
  assert.match(helper, /TarasevychGmailNotifierVersie00120260720V48Release/);
  assert.match(helper, /Invoke-GoogleJson DELETE .*legacyStaging/);
});

test('Versie 1 v48 isolates realtime with a per-lane lease and preserves card capacity guards', () => {
  ...
  assert.match(code, /GMAIL_NOTIFICATION_RUNTIME_CANDIDATE_ = 'v48'/);
